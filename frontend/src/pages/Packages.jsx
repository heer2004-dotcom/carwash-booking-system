...
const Packages = () => {
  ...
  return (
    <div className="container mx-auto p-6 max-w-3xl">
      ...
      {packages.length === 0 ? (
        <p className="text-slate-500">No packages available yet.</p>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {packages.map((p) => (
            <div key={p._id} className="card">
              <h2 className="text-xl font-bold">{p.name}</h2>
              <p className="text-slate-500 mb-3">{p.description}</p>
              <p className="text-2xl font-bold text-blue-600">${p.price}</p>
              <p className="text-sm text-slate-400">About {p.durationMinutes} mins</p>
            </div>
          ))}
        </div>
      )}
